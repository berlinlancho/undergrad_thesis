...
def step():
    ...
    in_ = ['已发布天数','历史日均评论增量','历史日均点赞增量','历史日均硬币增量',
...
if op == 1:
    ...
    if op == 0:
        ...
        resid = bigtable.sort_values(by='已发布天数')['re']
        ...
        for i in range(len(in_)):
            plt.subplot(3, 3, i + 1)
            plt.scatter(bigtable[in_[i]], bigtable['re'])
            plt.xlabel(in_[i])
        plt.subplot(3,3,7)
...
if op == 1:
    newdata = bigtable = pd.read_csv('../history/describe_result/bigtable_pre.csv')
    print(newdata.shape)
    rename(newdata)
    newdata['y_'] = lm_y.predict(newdata)
    newdata['y_fact'] = newdata['y_'].map(np.exp)
    newdata['re_fact'] = (newdata['y'].map(np.exp)- newdata['y_fact']).map(np.abs)
    newdata['re_log'] = (newdata['y'] - newdata['y_']).map(np.abs)
    datasize = newdata.shape[0]
    corect_fact = []
    corect_log = []
    e_rate = []
    ...
    for i in range(200):
        rate = i/80
        e_rate.append(rate)
        c_r_f = ((newdata['re_fact']/newdata['24h后播放增量']) < rate ).sum()/datasize
        c_r_l = ((newdata['re_log']/newdata['y']) < rate).sum()/datasize
        corect_fact.append(c_r_f)
        corect_log.append(c_r_l)
    ...
    for i in range(1,100):
        x1 = 2*(i-1)
        x2 = 2*i
        deta = corect_log[x2] - corect_log[x1]
        detas[deta] = x1
        if deta == 0 and len(extream_p) == 0:
            extream_p = e_rate[x1],corect_log[x1]

    turn_p = ()
    detas_l = list(detas.keys())
    dds = {}
    for i in range(1,len(detas_l)):
        d1 = i - 1
        d2 = i
        ddeta = detas_l[d2] - detas_l[d1]
        dds[detas_l[d1]] = ddeta

    min_ddl = 1
    min_k = 0
    for i in dds.keys():
        if np.abs(dds[i] - 0) < min_ddl:
            min_ddl = np.abs(dds[i] - 0)
            min_k = i

    turn_p = e_rate[detas[min_k]],corect_log[detas[min_k]]
    ...
    plt.figure(1)
    plt.subplot(1,2,1)
    plt.plot(e_rate,corect_fact)
    plt.xlabel('r_0')
    plt.ylabel('播放增量预测准确率')
    plt.subplot(1, 2, 2)
    plt.plot(e_rate[:70],corect_log[:70])
    plt.xlabel('r_0')
    plt.ylabel('播放增量对数预测准确率')
    plt.scatter(extream_p[0],extream_p[1],color='red')
    plt.scatter(turn_p[0],turn_p[1],color='green')
    plt.text(turn_p[0],turn_p[1]-0.05,f'拐点{turn_p}')
    plt.text(extream_p[0],extream_p[1]-0.05,f'驻点{extream_p}')
    plt.show()
    in_.extend(['标题','24h后播放增量','y','y_fact','y_','re_fact','re_log'])
    newdata[in_].to_excel('temp.xlsx')